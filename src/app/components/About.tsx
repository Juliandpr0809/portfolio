...
export function About() {
  ...
  return (
    <section id="about" className="bg-black text-white px-6 md:px-12 lg:px-24 py-32 lg:py-40">
      <div className="max-w-7xl mx-auto">
        <motion.div
          ...
          viewport={{ once: true }}
          className="grid lg:grid-cols-[1.5fr_1fr] gap-16 lg:gap-24"
        >
          {/* Left Column - Bio */}
          ...
          <div className="space-y-16">
            {/* Stats Section */}
            <div className="grid grid-cols-3 gap-4 p-8 bg-zinc-900/30 rounded-[2rem] border border-white/5">
              <div className="text-center">
                <span className="block text-4xl font-black text-white mb-1">3</span>
                <span className="text-[10px] uppercase font-bold tracking-widest text-zinc-500 leading-tight">Proyectos<br/>reales</span>
              </div>
              <div className="text-center">
                <span className="block text-4xl font-black text-white mb-1">2</span>
                <span className="text-[10px] uppercase font-bold tracking-widest text-zinc-500 leading-tight">Clientes<br/>reales</span>
              </div>
              <div className="text-center">
                <span className="block text-4xl font-black text-white mb-1 text-amber-500">8vo</span>
                <span className="text-[10px] uppercase font-bold tracking-widest text-zinc-500 leading-tight">Sem.<br/>Univ.</span>
              </div>
            </div>

            {/* Skills Section */}
            <div>
              <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500 mb-6 italic">Habilidades Core</h4>
              <div className="flex flex-wrap gap-2">
                {skills.map((skill) => (
                  <span 
                    key={skill}
                    className="px-4 py-2 rounded-full bg-zinc-900 border border-white/10 text-xs font-bold text-zinc-300 hover:border-amber-500/50 hover:text-white transition-colors cursor-default"
                  >
                    {skill}
                  </span>
                ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {timeline.map((item, index) => (
              <div key={index} className="flex gap-4 items-start">
                <span className="text-amber-500 font-black text-sm tabular-nums tracking-tighter">
                  {item.year} —
                </span>
                <span className="text-sm font-bold text-zinc-400 uppercase tracking-wide leading-tight">
                  {item.event}
                </span>
              </div>
            ))}
